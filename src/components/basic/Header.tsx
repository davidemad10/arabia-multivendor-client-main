...
export default function Header() {
  ...
  const setLoginPoup = () => {
    isLoginPoupOpen ? setIsLoginPoupOpen(false) : setIsLoginPoupOpen(true);
  };
  ...
  return (
    <header className="flexCenter">
      <div
        ...
        } duration-200  bg-white z-10 flex-col w-full max-md:hidden`}
      >
          ...
          <div className="container flexBetween flex">
            <div className=" w-11/12 gap-4 flex flexStart">
              <div className="flex flexCenter gap-2 bg-Red px-6 py-2 ">
                ...
                <span>
                  <TiArrowSortedDown className="text-xl text-white"></TiArrowSortedDown>{" "}
                </span>
              </div>
                ...
                {headerLinks.map((link) => {
                  ...
                  return (
                    <li key={link.route} className=" group">
                      <NavLink
                        className={`  hover:text-Red  pb-3 border-Red w-full h-full ${
                          isActive
        ...
        <LoginPoup
          isLoginPoupOpen={isLoginPoupOpen}
          setLoginPoup={setLoginPoup}
        ></LoginPoup>
      </div>
    </header>
  );
